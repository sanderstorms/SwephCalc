...
public interface IAzimuthCalculator
{
    ...
    /// <returns>Азумут верхнего и нижнего края солнка, а также промежуточные значения вычислений - <see cref="AzimuthCalcResult"/>.</returns>
    AzimuthCalcResult CalculateSunriseSunsetAzimuth(DateTime date,
    ...
    (HorizontalCoordinates Coordinates, double JulDay) GetSunriseSunsetAzimuthAndTime(DateTime date,
        GeoPosition position,
        double pressure,
        double temperature,
        int purpose);
    ...
    /// </param>
    /// <returns>Кординаты и точное время восхода/захода солнца.</returns>
    SunriseSunsetTimeCalcResult CalculateSunriseSunsetTime(DateTime date,
        GeoPosition position,
        double pressure,
        double temperature,
        int purpose);
...
{
    public AzimuthCalcResult CalculateSunriseSunsetAzimuth(DateTime date,
        GeoPosition position,
        double pressure,
        double temperature,
        int purpose,
        double KP)
    {
        ...
        if (purpose != SwephExp.SE_CALC_SET && purpose != SwephExp.SE_CALC_RISE)
        {
        ...
        if (position.Latitude >= AstroCatalogue.TableLatitudeValues[0] && position.Latitude <= AstroCatalogue.TableLatitudeValues[^1])
        {
            ...
            latitudeCorrection = GetLatitudeCorrection(baseAzimuthResult.Coordinates.Azimuth,
                rAzimuthResult.Coordinates.Azimuth,
                left,
                right,
                position.Latitude);
        }
        ...
        var longitudeCorrection = GetLongitudeCorrection(date,
            baseAzimuthResult.Coordinates.Azimuth,
            baseLatitude,
            position.Longitude,
            purpose);
        ...
        dA = purpose == SwephExp.SE_CALC_RISE ? dA : -dA;
    ...
    public (HorizontalCoordinates Coordinates, double JulDay) GetSunriseSunsetAzimuthAndTime(DateTime date,
        GeoPosition position,
        double pressure,
        double temperature,
        int purpose)
    ...
    /// </param>
    /// <returns>Кординаты и точное время восхода/захода солнца.</returns>
    public SunriseSunsetTimeCalcResult CalculateSunriseSunsetTime(DateTime date,
        GeoPosition position,
        double pressure,
        double temperature,
        int purpose)
    ...
    private (HorizontalCoordinates Coordinates, double JulDay) GetSunriseSunsetAzimuthAndTimeInternal(
        DateTime date,
        GeoPosition position,
        double pressure,
        double temperature,
        ...
        SweApi sweApi)
    {
        var jday = sweApi.SunriseSunsetJulDay(position, pressure, temperature, purpose, date);
        ...
        azimuth.Azimuth = azimuth.Azimuth >= 180
    ...
    private (HorizontalCoordinates Coordinates, double JulDay) GetStandardSunriseSunsetAzimuthAndTime(
        DateTime date,
        double latitude,
        int purpose)
    {
    ...
    private double GetAltitudeCorrection(double altitude)
    {
    ...
    private double GetLatitudeCorrection(double lAzimuth,
        double rAzimuth,
        double lLatitude,
        double rLatitude,
        double latitude)
    ...
    public double GetSunBotAgeCorrection(double latitude, double azimuth, int purpose)
    {
        ...
        return purpose == SwephExp.SE_CALC_RISE
            ? correction
            : -correction;
    }
